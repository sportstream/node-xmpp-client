...
var BasicClient = function(params, callback)
{
  ...
  this.xmpp.addListener('authFail', function()
  {
    ...
    callback.apply(jabber, {error: '[Error] Jabber : Authentication failure'});
//    process.exit(1);
  });
...
{
  this.xmpp.send(
    new xmpp.Element('presence', (type != null) ? {type: type} : {})
      .tree()
  );
};
...
BasicClient.prototype.iq = function(to, query, callback, error)
{
  if(error == undefined)
  {
    ...
    {
      sys.error((this.jid + " : " + stanza.toString()).red);
    };
  }
...
BasicClient.prototype.iqSet = function(to, query, callback, error)
{
  if(error == undefined)
  {
    error = function(stanza)
